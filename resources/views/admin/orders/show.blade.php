        ...
        <div class="bg-white shadow-md rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p><strong class="text-gray-700">ユーザー名:</strong> {{ $order->user->name ?? '不明' }}</p>
            ...
            {{-- 管理者用に追加: ステータス変更フォーム (コメントアウトされていますが、必要に応じて有効にできます) --}}
            {{-- <div class="mt-6">
                <h3 class="text-xl font-semibold mb-2">ステータス変更</h3>
                <form action="{{ route('admin.orders.update_status', $order) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <select name="status" id="status" class="form-select rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
