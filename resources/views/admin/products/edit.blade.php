                    ...
                    <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category_id" id="category_id" required
                                ...
                                @foreach ($categories as $mainCategory)
                                    <option value="{{ $mainCategory->id }}" {{ old('category_id', $product->category_id) == $mainCategory->id ? 'selected' : '' }}>
                                        {{ $mainCategory->name }}
                                    </option>
                                    @foreach ($mainCategory->children as $subCategory)
                                        ...
                                        @foreach ($subCategory->children as $deepSubCategory)
                                            <option value="{{ $deepSubCategory->id }}" {{ old('category_id', $product->category_id) == $deepSubCategory->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            @if ($product->image_url)
                                <div class="mt-1">
                                    <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="h-20 w-20 object-cover rounded-md">
                                </div>
                                <div class="mt-2">
                                    <label for="delete_image" class="inline-flex items-center">
                                        <input type="checkbox" name="delete_image" id="delete_image" value="1" class="rounded border-gray-300 text-red-600 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50">
                            ...
                            <label for="image" class="block text-sm font-medium text-gray-700 mt-4">新しい商品画像 (変更する場合)</label>
                        ...
                        <div class="mb-4">
                            <label for="is_limited" class="inline-flex items-center">
                                <input type="checkbox" name="is_limited" id="is_limited" value="1" {{ old('is_limited', $product->is_limited) ? 'checked' : '' }}
                        ...
                        <div class="mb-4" id="limited_location_field" style="{{ old('is_limited', $product->is_limited) ? '' : 'display:none;' }}">
                            ...
                            <select name="limited_location" id="limited_location"
                                ...
                                @foreach ($shops as $shop)
                                    <option value="{{ $shop->name }}" {{ old('limited_location', $product->limited_location) == $shop->name ? 'selected' : '' }}>
                                        {{ $shop->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4" id="limited_type_field" style="{{ old('is_limited', $product->is_limited) ? '' : 'display:none;' }}">
                            <label for="limited_type" class="block text-sm font-medium text-gray-700">限定タイプ</label>
                            <select name="limited_type" id="limited_type"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <option value="">選択してください</option>
                                <option value="location" {{ old('limited_type', $product->limited_type) == 'location' ? 'selected' : '' }}>店舗限定</option>
                                <option value="season" {{ old('limited_type', $product->limited_type) == 'season' ? 'selected' : '' }}>季節限定</option>
                            </select>
                        </div>
